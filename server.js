    ...
    jwt.verify(token.replace("Bearer ", ""), "secret_key", (err, decoded) => {
        ...
        db.query(classCountQuery, [decoded.id], (err, classResult) => {
            if (err.code === 'ER_DUP_ENTRY') {
                return res.status(400).send(`Error: ${error.sqlMessage}`);
            }
...
// Fetch last updated details
app.get("/api/get-last-updated", (req, res) => {
    const { branch_id } = req.query;

    let sql = `SELECT branch_name, excluded_roll_numbers, DATE_FORMAT(updated_at, '%Y-%m-%d %H:%i:%s') AS updated_at, previous_semester_id,
            upgraded_to FROM semester_updates`;
    let params = [];

    if (branch_id) {
        sql += " WHERE branch_id = ? ORDER BY updated_at DESC LIMIT 1";
    ...
    db.query(sql, params, (err, result) => {
        ...
        const { branch_name, excluded_roll_numbers, updated_at, previous_semester_id, upgraded_to } = result[0];

        // Format excluded roll numbers as a list
        const excludedList = excluded_roll_numbers !== "None"
            ? excluded_roll_numbers.split(", ").map(num => ` - ${num}`).join("\n")
            : "None";

        res.json({
            message: `Last updated on: ${updated_at}\nBranch: ${branch_name}\n Upgraded: Semester ${previous_semester_id} ➜ ${upgraded_to} \nExcluded Roll Numbers:\n${excludedList}`
        });
    });
});
        ...
        db.query(userQuery, [userId], (err, userResults) => {
            ...
            db.query(studentQuery, [email], (err, studentResult) => {
                ...
                db.query(reportQuery, params, (err, result) => {
                    ...
                    const totalPresent = result.reduce((sum, s) => sum + s.present_days, 0);
