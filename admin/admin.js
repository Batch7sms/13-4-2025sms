            ...
            data.forEach(regulation => {
                tableBody.innerHTML += `
                    <tr>
                        ...
                        <td>
                            <button onclick="deleteRegulation(${regulation.id})"><i class="fas fa-trash"></i></button>
                        </td>
                    </tr>
                `;
            });
            ...
            data.forEach(branch => {
                tableBody.innerHTML += `
                    <tr>
                        ...
                        <td>${branch.name}</td>
                        <td>
                            <button onclick="deleteBranch(${branch.id})"><i class="fas fa-trash"></i></button>
                        </td>
                    </tr>
                `;
            });
    ...
    paginatedAllocations.forEach(allocation => {
        allocationTable.innerHTML += `
            <tr>
                ...
                <td><button onclick="removeAllocation(${allocation.id})"><i class="fas fa-trash"></i></button></td>
            </tr>
        `;
    });

    document.getElementById("pagination1").innerHTML = `
        <button onclick="changePage1(-1)" ${currentPage === 1 ? "disabled" : ""}>
            <i class="fas fa-arrow-left"></i> 
        </button>
        Page ${currentPage} of ${totalPages || 1}
        <button onclick="changePage1(1)" ${currentPage >= totalPages ? "disabled" : ""}>
...
function renderPaginationn(totalSubjects) {
    ...
    paginationDiv.innerHTML = `
        <button onclick="changePages(-1)" ${currentPage === 1 ? "disabled" : ""}>
            <i class="fas fa-arrow-left"></i> 
        </button>
        Page ${currentPage} of ${totalPages}
        <button onclick="changePages(1)" ${currentPage >= totalPages ? "disabled" : ""}>
            <i class="fas fa-arrow-right"></i>
        </button>
    `;
}
...
async function loadLastUpdatedDate(branch_id = "") {
    ...
    data.message.forEach(msg => {
        ...
        updated.innerHTML = `<i class="fa-regular fa-clock"></i> ${lines[0]}`;
        ...
        excluded.className = "excluded-rolls";
        excluded.textContent = lines[lines.length - 1]; // Excluded Roll Numbers
